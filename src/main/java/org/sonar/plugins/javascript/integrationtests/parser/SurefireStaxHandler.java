...
public class SurefireStaxHandler implements StaxParser.XmlStreamHandler {
    ...
    private final IntegrationTestIndex index;

    public SurefireStaxHandler(IntegrationTestIndex index) {
        this.index = index;
    }
            ...
            if (testSuiteEvent.compareTo(SMEvent.START_ELEMENT) == 0) {
                ...
                for (event = testCase.getNext(); event != null; event = testCase.getNext()) {
                    if (event.compareTo(SMEvent.START_ELEMENT) == 0) {
                        ...
                        IntegrationTestClassReport classReport = index.index(testClassName);
    ...
    private void parseTestCase(SMInputCursor testCaseCursor, IntegrationTestClassReport report) throws XMLStreamException {
        report.add(parseTestResult(testCaseCursor));
    }

    private void setStackAndMessage(IntegrationTestResult result, SMInputCursor stackAndMessageCursor) throws XMLStreamException {
    ...
    private IntegrationTestResult parseTestResult(SMInputCursor testCaseCursor) throws XMLStreamException {
        IntegrationTestResult detail = new IntegrationTestResult();
        String name = getTestCaseName(testCaseCursor);
        detail.setName(name);

        String status = IntegrationTestResult.STATUS_OK;
        ...
        if (childNode.getNext() != null) {
            ...
            if ("skipped".equals(elementName)) {
                status = IntegrationTestResult.STATUS_SKIPPED;
                // bug with surefire reporting wrong time for skipped tests
                duration = 0L;

            } else if ("failure".equals(elementName)) {
                status = IntegrationTestResult.STATUS_FAILURE;
                setStackAndMessage(detail, childNode);

            } else if ("error".equals(elementName)) {
                status = IntegrationTestResult.STATUS_ERROR;
                setStackAndMessage(detail, childNode);
            }
        }
